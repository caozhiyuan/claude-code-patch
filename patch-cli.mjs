...
const importRe =
  /import\s*\{\s*execSync(?:\s+as\s+([A-Za-z_$][\w$]*))?\s*\}\s*from\s*["']child_process["'];/g;
const alreadyPatchedRe =
  /import\s+cp\$1\s+from\s*["']child_process["'];\s*const\s+[A-Za-z_$][\w$]*\s*=\s*\(\.\.\.A\)\s*=>\s*cp\$1\.execSync\(\.\.\.A\);/;
...
const patched = text.replace(importRe, () => {
  return `import cp$1 from"child_process";const ${alias}=(...A)=>cp$1.execSync(...A);`;
});
